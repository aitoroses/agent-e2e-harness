...
- The inspect artifacts carry an OC-grade UI forensics tree, not a flat ref list. `inspect.md` adds a page **Summary** (interactive/landmarks/headings/alerts/dialogs/forms/tables/images counts), document + viewport facts, a **What can I act on** table, and a hierarchical **UI tree**; `inspect.json` exposes the same as structured data. Each node carries factual geometry (box), visibility / hidden-ish / disabled flags, scroll dimensions+offset+overflow, layout (display / flex / grid / gap / position), and cheap typography/color/border facts. Referencable nodes carry their `@ref` so screenshot pixels map to act targets; the tree is capped (no DOM dump) and the `browser.refs` overlay still paints only referencable nodes to stay low-noise.
