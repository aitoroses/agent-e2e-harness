...
Expected checkpoint: `"status": "ok"` and the Dev MCP tools, including `stack.start`, `stack.list`, `stack.logs`, `stack.capability.list`, `stack.capability.run`, `stack.explore.list`, `stack.explore.run`, `run.begin`, `browser.open`, `browser.find`, `browser.wait`, `browser.get`, `browser.console`, `browser.network`, `browser.eval`, `browser.playwright`, `journey.step`, `artifact.read`, `cleanup.plan`, `run.teardown`, and `stack.stop`.
