...
Core harness code must not add Kubernetes, Docker, SQL, cloud, or shell-specific namespaces; products declare those diagnostics behind Runtime Exploration Tools. Declared shell diagnostics may exist as product-owned tools, but v1 does not expose arbitrary raw shell access.

Observation Runtime Exploration Tools run by default. `runMutation` tools require selected Journey Profile opt-in. `runtimeMutation` tools are blocked by default.
