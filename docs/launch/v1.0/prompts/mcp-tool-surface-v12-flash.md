# Prompt — mcp-tool-surface v12-flash (Browser Workbench + journey.untilPhase label fix)

Target render candidate: `/tmp/mcp-tool-surface-v12-flash.png`
...
Intent:
Create an updated README illustration for Agent E2E Harness after the Browser Workbench expansion. The old poster was an exact 19-tool inventory. This v12 Flash poster should teach the agent-facing mental model without becoming brittle: capability groups, not every individual tool as a row. Avoid wildcard labels because image renderers often distort `.*`.

v10 correction kept:
- The JOURNEY panel must include `journey.untilPhase`. The v9 render accidentally showed only `journey.phase`, which hides the important "run until a phase boundary" tool.

v11 correction:
- The ARTIFACT panel second row uses the shorter phrase "inspect prior evidence" to avoid renderer drift on the word "happened".

v12-flash correction:
- Regenerated with `gemini-3.1-flash-image-preview` at 2K after Pro renders drifted. Flash produced the accepted README candidate.
...
TITLE:
Top of frame, centered, bold uppercase technical sans-serif, near-black. Keep it compact: title occupies about 60% of the frame width, not edge-to-edge:
"AGENT CONTROLS"
...
LAYOUT:
Six labeled isometric tray-panels in a 3 x 2 grid with generous gutters. Every panel must be fully visible inside the canvas. Each panel header is uppercase near-black technical sans-serif. Within each panel, capability rows are stacked vertically. Each row is flat text, not a pill:
...
Panel 1 top-left — header "STACK" — five rows:
...
- `stack.explore.list` — discover provider-owned tools
- `stack.explore.run` — call one provider-owned tool
...
- `journey.phase` / `journey.untilPhase` — run or fast-forward through a phase

BOTTOM ROW:

Panel 4 bottom-left — header "BROWSER WORKBENCH" — largest bottom panel, six rows. Use slightly smaller monospace text in this panel so long labels fit:
...
- The image must contain exactly 22 capability rows total: STACK 5, RUN 3, JOURNEY 4, BROWSER WORKBENCH 6, ARTIFACT 2, CLEANUP 2.
- Tool labels must be spelled exactly, including dots, slashes, and asterisks:
  - `stack.start`, `stack.status`, `stack.logs`, `stack.explore.list`, `stack.explore.run`
  - `run.begin`, `run.reseed`, `run.teardown`
  - `journey.list`, `journey.inspect`, `journey.step`, `journey.phase` / `journey.untilPhase`
...
- Descriptions must match the row descriptions word-for-word, especially "inspect prior evidence".
- Descriptions are sentence case and have no trailing period.
- Critical exact label guard: render `stack.explore.list` and `stack.explore.run` exactly. Do not render `stack.explore,*`, `stack.explore*`, `stack.explore`, or any wildcard form.
