# Prompt — proof-loop v7 (applies v6 critic's three strong sharpenings A/B/C; differentiator 2)
...
- v5 → v6 (Aitor's differentiator-based redirect, full restart): dev-laptop+CI-card composition retired entirely; recast around time-travel debugging via artifacts.
- v6 → v7 (applies critic's three strong sharpenings on prompt-side hardening):
  - **Sharpening A:** added a CRITICAL ACCURACY block for the 6 station coordinates + 5 artifact filenames + 1 viewer-header label (12 load-bearing accuracy strings — drift = silent failure of the differentiator).
  - **Sharpening B:** JUMP BACK arc geometry tightened to "single half-ellipse opening downward, ONE peak ONE arrowhead at left landing point, NOT an S-curve / question-mark / U-bend / closed loop / partial spiral / bidirectional swap." One peak, one arrowhead, one direction.
  - **Sharpening C:** artifact viewer icon-squares specified as DRAWN GEOMETRIC SQUARES (clean 1px graphite outlined rectangles), NOT Unicode-glyph characters ('▢', '□', '◻'), which render inconsistently in diffusion models.

Original v5 → v6 framing:
...
THE BACK-JUMP ARC — the one explicit backward arrow in the launch set (sharpening B: arc geometry locked):
A single graphite (near-black) arc arrow rises FROM the cursor's apex at station 'P2.S3', arcs up and over the timeline as a SINGLE HALF-ELLIPSE opening downward, and lands DOWN at station 'P2.S2'. Geometry constraints (load-bearing — the arc IS the time-travel motion):
- The arc is a single smooth half-ellipse curve — ONE continuous arc with ONE peak at the top center and ONE landing point at the right end (cursor at 'P2.S3') and ONE landing point at the left end (arrowhead onto 'P2.S2').
- Exactly ONE triangular arrowhead at the LEFT end, pointing DOWN onto 'P2.S2'. NO second arrowhead at the right end (that end starts at the cursor apex with no arrowhead). NO double-headed arrow.
- The arc spans roughly the width of one station gap and peaks at about 1.2× the height of a station tile above the timeline. 2px solid graphite stroke, 1px shadow underneath for depth, NO other color.
- The arc is NOT an S-curve, NOT a question-mark hook, NOT a U-bend with two arrowheads, NOT a closed loop, NOT a partial spiral, NOT a bidirectional swap arrow, NOT a refresh icon. ONE peak, ONE arrowhead, ONE direction (right-to-left in screen-space, future-to-past in journey-time).
- A short label sits above the peak of the arc on a horizontal label band in small uppercase technical sans-serif near-black: 'JUMP BACK'. Two words only, no other text near the arc.
...
- Below the header, FIVE artifact rows. Each row is a flat horizontal entry (not a pill), with a small DRAWN GEOMETRIC SQUARE icon on the left, then the monospace filename, near-black, lowercase. The icon squares are DRAWN as actual rectangle geometry — clean 1px graphite outlined squares — NOT Unicode glyph characters (sharpening C). Diffusion models render Unicode-square glyphs ('▢', '□', '◻') inconsistently or as garbled text-shapes; the prompt requires actual drawn rectangles.

The five rows top-to-bottom:
  1. [drawn square icon] `before.png`
  2. [drawn square icon] `after.png`
  3. [drawn square icon] `failure.png`
  4. [drawn square icon] `console.json`
  5. [drawn square icon] `network.json`
...
The first three rows ('before.png', 'after.png', 'failure.png') are visual snapshots — their DRAWN icon-square may be filled with a faint cyan tint inside the square to suggest "image preview." The last two ('console.json', 'network.json') are log files — their DRAWN icon-square is graphite-outlined only with no fill.
...
CRITICAL ACCURACY — 11 load-bearing accuracy strings (sharpening A):

SIX station coordinates appear under the timeline in this exact order left-to-right:
- 'P1.S1', 'P1.S2', 'P1.S3', 'P2.S1', 'P2.S2', 'P2.S3'

Format is `P<digit>.S<digit>` for every label — uppercase P, uppercase S, digit 1-3 only, single dot separator. Six coordinates total, all unique, all in this order. The first four ('P1.S1' through 'P2.S1') carry mint-green checks; 'P2.S2' has a cyan halo (the inspected step); 'P2.S3' has the cursor parked above it (the current step). Re-verify spelling, count, and left-to-right order on every regen. Reject any regen that drifts a coordinate, swaps two stations, duplicates a label, drops the period, or substitutes punctuation.

FIVE artifact filenames appear inside the artifact viewer panel in this exact order top-to-bottom:
1. `before.png`
2. `after.png`
3. `failure.png`
4. `console.json`
...
Every filename is lowercase, has exactly ONE dot, ends with one of `.png` or `.json`. The first three are .png (image artifacts); the last two are .json (log artifacts). All five are universally Chrome-DevTools-recognizable — they ARE the differentiator. Reject any regen that drifts a filename (e.g. 'before.jpg' / 'screenshot.png' / 'result.json' / 'step-feedback.json'), repeats a filename, swaps two rows, or substitutes punctuation.

The artifact viewer header reads exactly 'STEP P2.S2 · ARTIFACTS' — middle-dot ('·' / U+00B7) separator, not a period, hyphen, slash, em-dash, or whitespace.

The above 6 coordinates + 5 filenames + 1 viewer-header label = 12 load-bearing accuracy strings. Drift = silent failure of the differentiator.
