# Prompt — hero-agent-journey v6 (differentiator 1: agent-centric tool design — map vs context-burning)
...
What changed from v5 (Aitor's differentiator-based redirect):
- v5's central concept ("same step, same result every time" via three pixel-identical echo tiles) MOVES to proof-loop v6 (time-travel debugging is differentiator 2). Echo-stack is REMOVED from the hero.
- v6's new central concept: **agent-centric tool design.** A coding agent navigates an Executable Journey with explicit coordinates and a small toolbelt — vs the alternative (context-burning, scrollback rediscovery, manual click-replay). The contrast is the message.
- Strap line replaces v5's "REPLAYS ANY JOURNEY STEP IN A FRESH ENVIRONMENT." New strap names the differentiator directly.
...
THE TOOLBELT — what makes this "agent-centric" load-bearing:
Floating just to the LEFT of and slightly ABOVE the terminal-prompt cursor, a small horizontal "tool palette" tile: a thin-outlined low-volume isometric strip about 30% the width of one phase platform, holding FOUR monospace tool-name chips stacked or listed inside, each chip rendered as flat text (no pills, no buttons), near-black on cream:
...
- A SINGLE small thin graphite arrow drawn as an awkward closed loop / question-mark hook — it starts in the scrollback area, loops back on itself once, and dead-ends without arriving anywhere. This single shape is the only loop in the entire image and it carries "going in circles" semantics.
