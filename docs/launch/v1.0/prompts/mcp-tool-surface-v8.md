# Prompt — mcp-tool-surface v8 (Browser Workbench + stable capability groups, stricter text fit)

Target render candidate: `/tmp/mcp-tool-surface-v8.png`
...
Intent:
Create an updated README illustration for Agent E2E Harness after the Browser Workbench expansion. The old poster was an exact 19-tool inventory. This v8 poster should teach the agent-facing mental model without becoming brittle: capability groups, not every individual tool as a row.
...
Panel 1 top-left — header "STACK" — four rows. The fourth label must be exactly `stack.explore.*`: period before the asterisk, no comma:
...
- `stack.explore.*` — run provider-owned inspection tools
...
- Critical exact label guard: render `stack.explore.*` with a period before the asterisk. Do not render `stack.explore,*`, `stack.explore*`, `stack.explore`, or `stack.explore . *`.
