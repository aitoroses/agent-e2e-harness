# Prompt — mcp-tool-surface v11 (Browser Workbench + journey.untilPhase label fix + artifact typo hardening)

Target render candidate: `/tmp/mcp-tool-surface-v11.png`
...
Intent:
Create an updated README illustration for Agent E2E Harness after the Browser Workbench expansion. The old poster was an exact 19-tool inventory. This v11 poster should teach the agent-facing mental model without becoming brittle: capability groups, not every individual tool as a row. Avoid wildcard labels because image renderers often distort `.*`.
