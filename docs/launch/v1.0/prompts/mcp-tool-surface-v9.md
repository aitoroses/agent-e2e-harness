# Prompt — mcp-tool-surface v9 (Browser Workbench + stable capability groups, no wildcard labels)

Target render candidate: `/tmp/mcp-tool-surface-v9.png`
...
Intent:
Create an updated README illustration for Agent E2E Harness after the Browser Workbench expansion. The old poster was an exact 19-tool inventory. This v9 poster should teach the agent-facing mental model without becoming brittle: capability groups, not every individual tool as a row. Avoid wildcard labels because image renderers often distort `.*`.
