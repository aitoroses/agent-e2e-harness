...
`apps/showcase/agent-e2e.config.ts` is the conventional showcase integration point for journeys, resource adapters, and the stack provider. `apps/showcase/scripts/dev-mcp.ts` is intentionally only a public entrypoint executed directly by Bun; `npm run dev:mcp` does not maintain a separate TypeScript compile/watch bridge or generated Node runtime. Showcase-specific stack/journey composition and its PostgreSQL Testcontainers provider live under `apps/showcase/src/harness/`; reusable lifecycle mechanics must remain in `@agent-e2e/harness`. Dev MCP uses a stable default MCP port and writes `.agents-e2e/dev-mcp.json`; app URLs remain stack-owned service data returned by `stack.start` / `stack.status`.
