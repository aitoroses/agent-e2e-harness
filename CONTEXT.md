...
- Planned for the later **Run Stack Binding** slice: journey runs that use a stack bind to a specific **Stack Instance** through `stackId`; subsequent journey execution and stack evidence resolve through that binding instead of a temporary ambient fallback.
- Planned for the later **Run Stack Binding** slice: when a **Stack Provider** is configured, `run.begin` requires `stackId`; when no **Stack Provider** is configured, `stackId` is invalid rather than ignored.
- Planned for later stack evidence capture: `stack.logs` and `stack.explore.run` may accept optional `runId` only for artifact capture and must validate that the run's **Run Stack Binding** matches the target `stackId`.
